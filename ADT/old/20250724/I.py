...
def solve(n, a_list):
    cnt_tree = SegmentTree(n, add, e=lambda: 0)
    sum_tree = SegmentTree(n, add, e=lambda: 0)
    a_list_s = list(sorted([(x, i) for i, x in enumerate(a_list)]))
    # print(a_list_s)
    a_list_st = []
    for j in range(n):
        x, i = a_list_s[j]
        a_list_st.append((i, x, j))
    a_list_st = list(sorted(a_list_st))
    # print(a_list_st)
    res = 0
    for i in range(n - 1, -1, -1):
        _, x, j = a_list_st[i]
        c = cnt_tree.query(j, n)
        s = sum_tree.query(j, n)
        res += s - c * x
        cnt_tree.set_val(j, 1)
        sum_tree.set_val(j, x)
        # print(res)
    return res
...
def test():
    assert solve(3, [2, 5, 3]) == 4
    assert solve(10, [5, 9, 3, 0, 4, 8, 7, 5, 4, 0]) == 58
