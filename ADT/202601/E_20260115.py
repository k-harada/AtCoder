...
def solve(n, q, s, lr_list):
    ...
    for i in range(n - 1):
        if s[i] == s[i + 1]:
            x_list[i] = 1
    sgt = SegmentTree(n, op=lambda x, y: x + y, e=lambda: 0)
    sgt.initialize(x_list)
    for l, r in lr_list:
        res.append(sgt.query(l - 1, r - 1))
    # print(res)
    return res
...
def test():
    assert solve(11, 4, "mississippi", [
        (3, 9),
        (4, 10),
        (4, 6),
        (7, 7)
    ]) == [2, 2, 0, 0]
    assert solve(5, 1, "aaaaa", [(1, 5)]) == [4]
