...
def solve(n, a_list):
    m = max(a_list) + 1
    sgt_cnt = SegmentTree(m, op=add_mod, e=lambda: 0)
    sgt_cnt.initialize([0] * m)
    sgt_sum = SegmentTree(m, op=add_mod, e=lambda: 0)
    sgt_sum.initialize([0] * m)
    ...
    for i, a in enumerate(a_list):
        ...
        v += a * (2 * sgt_cnt.query(0, a + 1) + 1)

        c = sgt_cnt.query(a, a + 1)
        sgt_cnt.set_val(a, c + 1)

        v += 2 * sgt_sum.query(a + 1, m)

        b = sgt_sum.query(a, a + 1)
        sgt_sum.set_val(a, b + a)

        v %= MOD

        # print(v)

        r = v * pow((i + 1), MOD - 3, MOD)
        r %= MOD
...
def test():
    ...
    assert solve(7, [22, 75, 26, 45, 72, 81, 47]) == [22, 249561150, 110916092, 873463862, 279508479, 360477194, 529680742]


def test_large():
    print(solve(200000, list(range(1, 200001))))
