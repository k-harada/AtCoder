...
def solve(n, c, a_list):
    if c > 0:
        ...
        for a in a_list:
            a_sum.append(a_sum[-1] - a)
    sgt = SegmentTree(n + 1, op=max, e=lambda: -10 ** 13)
    sgt.initialize(a_sum)
    s = sum(a_list)
    res = s
    for i in range(n):
        r_max = sgt.query(i, n + 1) - a_sum[i]
        if c > 0:
            res = max(res, s + (c - 1) * r_max)
        else:
            res = max(res, s + (1 - c) * r_max)
    return res
...
def main():
    ...
    res = solve(n, c, a_list)
    print(res)


def test():
    assert solve(5, 2, [-10, 10, 20, 30, -20]) == 90
    assert solve(5, 1000000, [-1, -2, -3, -4, -5]) == -15
    assert solve(9, -1, [-9, 9, -8, 2, -4, 4, -3, 5, -3]) == 13
